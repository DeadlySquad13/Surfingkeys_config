...
const paths = {
  ...
  get scripts() {
    return [
      ...
      "keys.js",
      "util.js",
    ].map((script) => path.join(this.scriptsDir, script))
  },
  compatibilityLayer: "compatibilityLayer.js", // For surfingkeys#1.x^ compatibility.
  get entry() {
    return path.join(this.scriptsDir, "conf.js")
  },
  ...
  buildDir:         "Surfingkeys/",
  // installDir:       platforms.getConfigHome(),
  installDir:       "E:/Configs/",
}
...
const loadFaviconsManifest = async () => {
  ...
  try {
    ...
    faviconsManifest = require(`./${path.join(
      paths.favicons,
      paths.faviconsManifest,
    )}`)
  } catch (e) {
    ...
    .pipe(
      eslint.results((res) => {
        if (res.warningCount + res.errorCount > 0) {
          ...
          const numBadFiles = res.reduce(
            (acc, e) => (e.errorCount + e.warningCount > 0 ? acc + 1 : acc),
            0,
          )
...
task("lint-fix", () =>
  lint([...paths.scripts, paths.gulpfile], { fix: true }).pipe(
    gulpIf(
...
task("check-priv", async () => {
  try {
    ...
    console.log(
      "Notice: Initializing ./conf.priv.js - configure your API keys here.",
    )
    return fs.copyFile(
      "./conf.priv.example.js",
      "./conf.priv.js",
      COPYFILE_EXCL,
    )
  }
...
task(
  ...
  parallel(async () => {
    ...
    const screenshots = await fs.readdir(
      path.join(__dirname, paths.screenshots),
    )
  ...
  series("clean-favicons", async () => {
    ...
    const sites = []
      .concat(
        ...
        Object.entries(compl).map(([, v]) => ({
          domain:  new URL(v.domain ? `https://${v.domain}` : v.search).hostname,
          favicon: `https://icons.duckduckgo.com/ip3/${new URL(v.domain ? `https://${v.domain}` : v.search).hostname
          }.ico`,
        })),
        ...
        Object.keys(keys.maps)
          ...
          .map((k) => ({
            domain:  k,
            favicon: `https://icons.duckduckgo.com/ip3/${new URL(`https://${k}`).hostname
            }.ico`,
          })),
      )
    ...
    return file(files).pipe(dest(paths.favicons))
  }),
)
...
task(
  "install",
  series("build", () =>
    src(path.join(paths.buildDir, paths.scriptOut)).pipe(dest(paths.installDir))),
)
...
task(
  "watch-docs-full",
  watch([...paths.scripts, paths.readme], series("docs-full")),
)
